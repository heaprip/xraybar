...
выделите QR-код — снимок попадёт в буфер обмена) или **Import from v2rayN…** (только чтение). **Share Server…** показывает QR-код. Правый клик по серверу или набору правил в панели позволяет
удалить его.
