...
> Статус: ранний; автор пользуется им каждый день. Ещё не сделано: учётные данные в Keychain,
> IPv6 через туннель, проверенный в реальной IPv6-сети, проверенное поведение при сне/пробуждении и смене сети, транспорты кроме raw.
...
2. **Скачайте Xray и данные маршрутизации.** *Xray › Download v26.9.9* и *Update Routing Data*
   (с проверкой контрольных сумм). Установка Xray один раз спрашивает пароль: он попадает в
   каталог, принадлежащий root, — единственное место, откуда XrayBar запускает его от root.
   Если установлен v2rayN, *Copy Xray from v2rayN…* делает то же с его копией; данные
...
Верить на слово не нужно. Приложение — это около 1500 строк Swift в нескольких пронумерованных файлах, которые
читаются по порядку, плюс около 250 строк root-скриптов и необязательный root-помощник
