...
| As root | copies the config to `/var/run/xraybar` and replaces its log section (stdout only), starts xray (only a root-owned version, see below), sets DNS, waits, stops xray, restores DNS; `--restore` cleans up after a session that died | same script |
