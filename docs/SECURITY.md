...
| Files read | the above; optionally v2rayN's database for one-time import (read-only); the clipboard only when you choose *Import Link or QR Code from Clipboard* (QR decoded on-device by Vision; the app never captures the screen) | `3-Import.swift`, `6-Menu.swift` |
