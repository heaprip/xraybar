...
| Files read | the above; optionally v2rayN's database for one-time import (read-only); the clipboard, or one window/display you pick in the system picker, only when you choose an import command (QR decoded on-device by Vision; no Screen Recording permission is held) | `3-Import.swift`, `6-Menu.swift` |
